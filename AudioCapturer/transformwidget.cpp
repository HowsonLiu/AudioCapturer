...
TransformWidget::TransformWidget(QWidget *parent) : QWidget(parent)
{
    ...
    connect(pok_btn_,SIGNAL(clicked(bool)),this,SLOT(OKClick_Slot()));
}
...
void TransformWidget::OKClick_Slot()
{
    if(psave_edit_->text().isEmpty()){
        QMessageBox mes(QMessageBox::Critical,tr("Error"),tr("Path can not be empty"),QMessageBox::Ok, nullptr, Qt::FramelessWindowHint);
    ...
    QString format;
    if(pwav_rbtn_->isChecked()){
        format = "WAV";
    }
    emit Transform_Signal(format,psave_edit_->text());
}
