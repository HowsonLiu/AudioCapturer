...
void NewDialog::Start_Slot()
{
    ...
    if(psettime_check_->isChecked()){
        ...
        SetCountdownToStart(ptime_widget_->GetStartTime(),pcapture_dialog_->get_isstart());
        SetCountdownToStop(ptime_widget_->GetStopTime(),pcapture_dialog_->get_isstop_());
    }
    else{
    ...
    if(psetformat_check_->isChecked()){
        ...
        SetCustomFormat(pformat_widget_->get_formattag(),
                        pformat_widget_->get_channels(),
                        pformat_widget_->get_samplepersec(),
                        pformat_widget_->get_avgbytepersec(),
                        pformat_widget_->get_blockalign(),
                        pformat_widget_->get_bitspersample(),
                        pformat_widget_->get_cbsize(),
                        pformat_widget_->get_sample(),
                        pformat_widget_->get_dwchannelmask(),
                        pformat_widget_->get_subformat().toStdString().c_str());
    }
    else{
    ...
    qDebug() << "get path from name and get capture format";
    path = ChangeCharpToQString(GetPathByName(pname_edit_->text().toStdString().c_str()));
    if(pmicro_rbtn_->isChecked()){
        format = "Microphone";
    }
    else if(ploopback_rbtn_->isChecked()){
        format = "Loopback";
    }

    qDebug() << "judge file existed or object in list.";
    if(IsCAFileExist(path.toStdString().c_str())||IsCAObjectInList(path.toStdString().c_str())){
