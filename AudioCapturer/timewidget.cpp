...
TimeWidget::TimeWidget(QWidget *parent) : QWidget(parent)
{
    ...
    pcdstart_timeedit_->setDisplayFormat("m:ss");
    pcdstop_timeedit_->setDisplayFormat("m:ss");
...
long TimeWidget::GetStartTime()
{
    long sec = pcdstart_timeedit_->time().minute() * 60 + pcdstart_timeedit_->time().second();
    return sec * 1000;
}

long TimeWidget::GetStopTime()
{
    long sec = pcdstop_timeedit_->time().minute() * 60 + pcdstop_timeedit_->time().second();
    return sec * 1000;
}
