...
void MainDialog::Open_Slot()
{
    ...
    if(QFile(path).open(QFile::ReadOnly)){
        qDebug() << "judge file is in list?Yes nothing. No!" ;
        qDebug() << "New file in dll" ;
        qDebug() << "Read file stream";
        AddObjectToList(path);
    }
}
...
void MainDialog::Double_Click_Object_Slot(QListWidgetItem* pitem)
{
    if(pobject_dialog_ == nullptr){
        ...
        connect(pobject_dialog_,SIGNAL(Delete_Signal(const QString)),this,SLOT(DeleteObject_Slot(const QString)));
    ...
    pobject_dialog_->SetCurrentObject(pwidget->get_filepath());
    pobject_dialog_->UpdateObjectInfo();
...
void MainDialog::AddObjectToList(const QString path)
{
    QListWidgetItem* pitem = new QListWidgetItem(plist_);
    ObjectMiniWidget* pwidget = new ObjectMiniWidget(path,this);
    connect(pwidget,SIGNAL(DeletebtnClick_Signal(const QString)),this,SLOT(DeleteObject_Slot(const QString)));
...
void MainDialog::DeleteObject_Slot(const QString path)
{
